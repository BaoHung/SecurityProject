...
int main(int argc, char **argv)
{
    ...
    if (fileContent)
    {
        ...
        printf("Decrypted\n");
        printf("Unpadding...\n");
        paddingLength = fullOutput[fileLength - 1];
        finalLength = fileLength - paddingLength;
        printf("Padded length: %d ", fullOutput[fileLength - 1]);
        printf("\nUnpadded\n");
        ...
        {
            printf("Writing to %s ...\n", argv[2]);
            fwrite(fullOutput, 1, finalLength, outputFile);
            fclose(outputFile);
        }
    }
    printf("\nExit\n");
